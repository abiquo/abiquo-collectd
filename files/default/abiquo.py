...
def abiquo_init():
    collectd.register_write(abiquo_write, data={ 'lock' : threading.Lock(), 'last_flush' : get_current_time(), 'values' : [] })
...
def parse_types_file(file):
    ...
    for line in types:
        ...
        for ds in fields[1:]:
            ...
            if len(ds_fields) != 4:
                collectd.warning("%s: cannot parse data source '%s' on type '%s'" \
...
def abiquo_write(value_list, data):
    ...
    if length == 0 or (last_flush < config['flush_interval_secs'] and length < config['flush_max_values']):
        data['lock'].release()
        return
